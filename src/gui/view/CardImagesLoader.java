...
import gui.controller.CardEnumeration;
...
/**
 * Load up all the images for a specific deck of cards (or the default if none
 * selected).
 * <p>
 * 
...
public class CardImagesLoader implements Runnable {
	...
	protected String deckType = "cards";
	...
	/**
	 * Return CardImages object for this deck (whether in cache or freshly
	 * loaded) using given ICardImageStatus observer.
	 * <p>
	 * If the deck is to be loaded, then peer is used as an
	...
	public static CardImages getDeck(Component peer) {
		...
		while (!cil.ready()) {
			try {
				...
				System.err
						.println("CardImagesLoader::getDeck(). Unable to completely load deck: "
								+ defaultName);
				break;
			}
		}
	 ...
	 */
	public void run() {
		// The media Tracker ensures all images are fully loaded: This avoids
		// the
		// arbitrary race conditions that may happen when running a plugin
		// before all
		...
		while (ce.hasMoreElements()) {
			...
			try {
				File f = new File(imageDirectory + deckType + "/" + key
						+ ".gif");
		...
		try {
			File f = new File(imageDirectory + deckType + "/"
					+ backResourceName + ".gif");
			java.net.URL url = f.toURI().toURL();
			Image img = java.awt.Toolkit.getDefaultToolkit().getImage(url);
			mt.addImage(img, idx++);
		...
		// keep around so thread can return this value in synchronized method
		// getLoadedCardImages
		loadedImages = ci;
